...
export async function POST(
    ...
    try {
        ...
        const uploadsDir = join(process.cwd(), "uploads", "hiring-managers", id);
        ...
        const relativePath = `uploads/hiring-managers/${id}/${fileName}`;

        // Make a request to backend API to save document metadata
        const apiUrl = process.env.API_BASE_URL || "http://localhost:8080";
        const response = await fetch(`${apiUrl}/api/hiring-managers/${id}/documents`, {
            method: "POST",
            headers: {
