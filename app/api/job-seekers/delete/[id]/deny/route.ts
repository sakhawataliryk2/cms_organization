...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(`${apiUrl}/api/job-seekers/delete/${id}`, {
      method: "GET",
      headers: {
...
export async function POST(
  ...
  try {
    ...
    const apiUrl = process.env.API_BASE_URL || "http://localhost:8080";
    const response = await fetch(`${apiUrl}/api/job-seekers/delete/${id}/deny`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
    ...
    console.error("Error denying job seeker delete request:", error);
    return NextResponse.json(
      {
        success: false,
