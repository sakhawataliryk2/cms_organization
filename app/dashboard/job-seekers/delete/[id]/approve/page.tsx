...
export default function ApproveJobSeekerDeletePage() {
  ...
  useEffect(() => {
    ...
    const fetchRequestDetails = async () => {
      try {
        ...
        const response = await fetch(`/api/job-seekers/delete/${deleteRequestId}`, {
          method: "GET",
          headers: {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const timeoutId = setTimeout(() => controller.abort(), APPROVE_REQUEST_TIMEOUT_MS);
      ...
      if (error instanceof Error && error.name === "AbortError") {
        setStatus("error");
        setMessage("Request timed out. The server may be slow. Please try again or contact support.");
      } else {
        setStatus("error");
