...
out.sort((a, b) => {
  ...
  const renderDetailsPanel = () => {
    ...
    const effectiveRows = detailsKeys.map((key) => ({
      key,
    ...
    return (
      <PanelWithHeader title="Organization Details" onEdit={() => handleEditPanel("organizationDetails")}>
        <LookupEntityDetailsGrid
          ...
          recordId={organizationIdFromField3}
          visibleKeys={visibleFields.organizationDetails || []}
          emptyPlaceholder="-"
        />
      </PanelWithHeader>
    );
    ...
    <div className="bg-gray-200 min-h-screen p-2">
      ...
      {showAddNote && hiringManager && (
        <AddNoteModal
          ...
          entityId={hiringManagerId ?? ""}
          entityDisplay={hiringManager.fullName || `Hiring Manager #${hiringManagerId}`}
          onSuccess={() => { if (hiringManagerId) fetchNotes(hiringManagerId); }}
        />
      )}
