...
export default function AddLead() {
  ...
  // Scraping state (only for modify/edit view)
  const [isScraping, setIsScraping] = useState(false);
  const [showScrapeModal, setShowScrapeModal] = useState(false);
  const [scrapedLeads, setScrapedLeads] = useState<any[]>([]);
  const [selectedScrapedLead, setSelectedScrapedLead] = useState<number | null>(null);
  const [scrapeUrl, setScrapeUrl] = useState("https://toponehire.com");
  ...
  // Scraping functions (only for modify/edit view)
  const handleScrape = async () => {
    setIsScraping(true);
    setError(null);
    setScrapedLeads([]);
    setSelectedScrapedLead(null);

    try {
      const response = await fetch("/api/scrape/toponehire", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleUseScrapedLead = (index: number) => {
    ...
    setFormData((prev) => ({
      ...
      organizationId: lead.organization || prev.organizationId,
      linkedinUrl: lead.linkedinUrl || prev.linkedinUrl,
    }));
    ...
    if (lead.firstName) {
      setCustomFieldValues((prev) => ({
        ...prev,
        ...(customFields.find((f) => f.field_label === "First Name") && {
          [customFields.find((f) => f.field_label === "First Name")!.field_name]: lead.firstName,
        }),
      }));
    }
    if (lead.lastName) {
      setCustomFieldValues((prev) => ({
        ...prev,
        ...(customFields.find((f) => f.field_label === "Last Name") && {
          [customFields.find((f) => f.field_label === "Last Name")!.field_name]: lead.lastName,
        }),
      }));
    }
    if (lead.email) {
      setCustomFieldValues((prev) => ({
        ...prev,
        ...(customFields.find((f) => f.field_label === "Email") && {
          [customFields.find((f) => f.field_label === "Email")!.field_name]: lead.email,
        }),
      }));
    }
    if (lead.phone) {
      setCustomFieldValues((prev) => ({
        ...prev,
        ...(customFields.find((f) => f.field_label === "Phone") && {
          [customFields.find((f) => f.field_label === "Phone")!.field_name]: lead.phone,
        }),
      }));
    }
    if (lead.title) {
      setCustomFieldValues((prev) => ({
        ...prev,
        ...(customFields.find((f) => f.field_label === "Title") && {
          [customFields.find((f) => f.field_label === "Title")!.field_name]: lead.title,
        }),
      }));
    }
    if (lead.organization) {
      setCustomFieldValues((prev) => ({
        ...prev,
        ...(customFields.find((f) => f.field_label === "Organization") && {
          [customFields.find((f) => f.field_label === "Organization")!.field_name]: lead.organization,
        }),
      }));
    }
    if (lead.linkedinUrl) {
      setCustomFieldValues((prev) => ({
        ...prev,
        ...(customFields.find((f) => f.field_label === "LinkedIn URL") && {
          [customFields.find((f) => f.field_label === "LinkedIn URL")!.field_name]: lead.linkedinUrl,
        }),
      }));
    }
    if (lead.location) {
      setCustomFieldValues((prev) => ({
        ...prev,
        ...(customFields.find((f) => f.field_label === "Location") && {
          [customFields.find((f) => f.field_label === "Location")!.field_name]: lead.location,
        }),
      }));
    }

    setSelectedScrapedLead(index);
  ...
  return (
    <div className="mx-auto py-4 px-4 sm:py-8 sm:px-6">
      <div className="bg-white rounded-lg shadow p-4 sm:p-6 relative">
        ...
        <div className="flex justify-between items-center border-b pb-4 mb-6">
          ...
          <div className="flex items-center space-x-4">
            {/* Scrape button - only visible on modify/edit view */}
            <button
              type="button"
      ...
      {/* Scrape Modal - Only visible on modify/edit view */}
      {showScrapeModal && (
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-6xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="bg-gray-100 p-4 border-b flex justify-between items-center sticky top-0">
              <h2 className="text-xl font-semibold text-gray-800">
                Scrape Leads from Toponehire.com
              </h2>
              <button
                onClick={() => {
                  setShowScrapeModal(false);
                  setScrapedLeads([]);
                  setSelectedScrapedLead(null);
                  setScrapeUrl("https://toponehire.com");
                  setScrapeQuery("");
                }}
                className="text-gray-500 hover:text-gray-700"
                disabled={isScraping}
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="https://toponehire.com"
                    disabled={isScraping}
                  />
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    <h3 className="text-lg font-semibold">
                      Found {scrapedLeads.length} Lead(s) - Select one to populate form
                    </h3>
                  </div>
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {scrapedLeads.map((lead, index) => (
                          <tr
                            key={index}
                            className={
                              selectedScrapedLead === index
                                ? "bg-blue-50"
                                : "hover:bg-gray-50 cursor-pointer"
                            }
                            onClick={() => handleUseScrapedLead(index)}
                          >
                            <td className="px-4 py-3">
                              <input
                                type="radio"
                                name="scrapedLead"
                                checked={selectedScrapedLead === index}
                                onChange={() => handleUseScrapedLead(index)}
