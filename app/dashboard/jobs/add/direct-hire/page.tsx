...
export default function AddDirectHireJob() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        const orgField = customFields.find(
          (f) => String(f.field_label || "").trim().toLowerCase() === "organization"
        );
        if (orgField) {
          setCustomFieldValues((prev) => {
            if (prev[orgField.field_name]) return prev;
            return { ...prev, [orgField.field_name]: orgId };
          });
        }
      } catch (e) {
        console.error("Prefill org from hiring manager failed (direct-hire):", e);
      }
    };
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error("Sidebar job parse (direct-hire):", err);
      setParseJobError(
